...
static hio_ds18b20_sensor_t ds18b20_sensors[DS18B20_SENSOR_COUNT];

void ds18b20_event_handler(hio_ds18b20_t *self, uint64_t device_address, hio_ds18b20_event_t e, void *p)
{
    (void) p;
    ...
    if (e == HIO_DS18B20_EVENT_UPDATE)
    {
        hio_ds18b20_get_temperature_celsius(self, device_address, &temperature);

        device_index = hio_ds18b20_get_index_by_device_address(self, device_address);

        hio_log_debug("UPDATE %" PRIx64 "(%d) = %f", device_address, device_index, temperature);
    }
    else
    {
...
void application_init(void)
{
    ...
    hio_ds18b20_init_multiple(&ds18b20, ds18b20_sensors, DS18B20_SENSOR_COUNT, HIO_DS18B20_RESOLUTION_BITS_12);
