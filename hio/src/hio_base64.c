...
const char hio_b64_alphabet[] = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";

static uint8_t hio_base64_lookup(char c);
...
bool hio_base64_encode(char *output, size_t *output_length, uint8_t *input, size_t input_length)
{
    ...
    for (; input_length != 0; input_length--)
    {
        ...
        if (i == 3)
        {
            ...
            for (i = 0; i < 4; i++)
            {
                output[encode_length++] = hio_b64_alphabet[a4[i]];
    ...
    if (i != 0)
    {
        ...
        for (j = 0; j < (i + 1); j++)
        {
            output[encode_length++] = hio_b64_alphabet[a4[j]];
            if (encode_length > *output_length)
            {
                return false;
            }
