...
#define _HIO_HTS221_DELAY_RUN 50
#define _HIO_HTS221_DELAY_INITIALIZATION 50
#define _HIO_HTS221_DELAY_MEASUREMENT 50

static void _hio_hts221_task_interval(void *param);

static void _hio_hts221_task_measure(void *param);

static bool _hio_hts221_load_calibration(hio_hts221_t *self);
...
void hio_hts221_init(hio_hts221_t *self, hio_i2c_channel_t i2c_channel, uint8_t i2c_address)
{
    ...
    self->_task_id_interval = hio_scheduler_register(_hio_hts221_task_interval, self, HIO_TICK_INFINITY);
    self->_task_id_measure = hio_scheduler_register(_hio_hts221_task_measure, self, _HIO_HTS221_DELAY_RUN);

    self->_tick_ready = _HIO_HTS221_DELAY_RUN;

    hio_i2c_init(self->_i2c_channel, HIO_I2C_SPEED_400_KHZ);

    // TODO This delays initialization, should be part of state machine
    _hio_hts221_load_calibration(self);
}
...
start:
    ...
    switch (self->_state)
    {
        case HIO_HTS221_STATE_ERROR:
        {
            ...
            {
                self->_event_handler(self, HIO_HTS221_EVENT_ERROR, self->_event_param);
            }

            self->_state = HIO_HTS221_STATE_INITIALIZE;

            return;
        }
        case HIO_HTS221_STATE_INITIALIZE:
        {
            self->_state = HIO_HTS221_STATE_ERROR;

            uint8_t ctrl_reg1;

            if (!hio_i2c_memory_read_8b(self->_i2c_channel, self->_i2c_address, HTS221_CTRL_REG1, &ctrl_reg1))
            {
                goto start;
            }
            ...
            if (!hio_i2c_memory_write_8b(self->_i2c_channel, self->_i2c_address, HTS221_CTRL_REG1, ctrl_reg1))
            {
                goto start;
            }

            self->_state = HIO_HTS221_STATE_MEASURE;

            self->_tick_ready = hio_tick_get() + _HIO_HTS221_DELAY_INITIALIZATION;
        ...
        case HIO_HTS221_STATE_MEASURE:
        {
            ...
            if (!hio_i2c_memory_write_8b(self->_i2c_channel, self->_i2c_address, HTS221_CTRL_REG1, HTS221_BIT_PD | HTS221_BIT_BDU))
            {
                goto start;
            }

            if (!hio_i2c_memory_write_8b(self->_i2c_channel, self->_i2c_address, HTS221_CTRL_REG2, HTS221_BIT_ONE_SHOT))
            {
                goto start;
            }

            self->_state = HIO_HTS221_STATE_READ;

            hio_scheduler_plan_current_from_now(_HIO_HTS221_DELAY_MEASUREMENT);

            return;
        }
        case HIO_HTS221_STATE_READ:
        {
            self->_state = HIO_HTS221_STATE_ERROR;
            ...
            if (!hio_i2c_memory_read_8b(self->_i2c_channel, self->_i2c_address, HTS221_STATUS_REG, &reg_status))
            {
                goto start;
            }
            ...
            if (!hio_i2c_memory_read_8b(self->_i2c_channel, self->_i2c_address, HTS221_HUMIDITY_OUT_H, &retval[1]))
            {
                goto start;
            }

            if (!hio_i2c_memory_read_8b(self->_i2c_channel, self->_i2c_address, HTS221_HUMIDITY_OUT_L, &retval[0]))
            {
                goto start;
            }
            ...
            self->_state = HIO_HTS221_STATE_UPDATE;

            goto start;
        }
        case HIO_HTS221_STATE_UPDATE:
        {
            self->_measurement_active = false;

            if (self->_event_handler != NULL)
            {
                self->_event_handler(self, HIO_HTS221_EVENT_UPDATE, self->_event_param);
            }
...
static bool _hio_hts221_load_calibration(hio_hts221_t *self)
{
    ...
    for (i = 0; i < 16; i++)
    {
        if (!hio_i2c_memory_read_8b(self->_i2c_channel, self->_i2c_address, HTS221_CALIB_OFFSET + i, &calibration[i]))
        {
            return false;
        }
    }
