...
static void _hio_sam_m8q_task(void *param);
static bool _hio_sam_m8q_parse(hio_sam_m8q_t *self, const char *line);
static bool _hio_sam_m8q_feed(hio_sam_m8q_t *self, char c);
static void _hio_sam_m8q_clear(hio_sam_m8q_t *self);
static bool _hio_sam_m8q_enable(hio_sam_m8q_t *self);
static bool _hio_sam_m8q_disable(hio_sam_m8q_t *self);
static bool _hio_sam_m8q_send_config(hio_sam_m8q_t *self);

void hio_sam_m8q_init(hio_sam_m8q_t *self, hio_i2c_channel_t channel, uint8_t i2c_address, const hio_sam_m8q_driver_t *driver)
{
    memset(self, 0, sizeof(hio_sam_m8q_t));
    ...
    self->_task_id = hio_scheduler_register(_hio_sam_m8q_task, self, HIO_TICK_INFINITY);
}
...
static void _hio_sam_m8q_task(void *param)
{
    hio_sam_m8q_t *self = param;

    if (!self->_running)
    {
        self->_state = HIO_SAM_M8Q_STATE_STOP;
    }

    if (self->_running && self->_state == HIO_SAM_M8Q_STATE_STOP)
    {
        self->_state = HIO_SAM_M8Q_STATE_START;
    }
...
start:
    ...
    switch (self->_state)
    {
        case HIO_SAM_M8Q_STATE_ERROR:
        {
            if (self->_event_handler != NULL)
            {
                self->_event_handler(self, HIO_SAM_M8Q_EVENT_ERROR, self->_event_param);
            }
        ...
        case HIO_SAM_M8Q_STATE_START:
        {
            if (!_hio_sam_m8q_enable(self))
            {
                self->_state = HIO_SAM_M8Q_STATE_ERROR;

                goto start;
            }

            _hio_sam_m8q_clear(self);

            if (self->_event_handler != NULL)
            {
                self->_event_handler(self, HIO_SAM_M8Q_EVENT_START, self->_event_param);
            }

            self->_state = HIO_SAM_M8Q_STATE_READ;

            hio_scheduler_plan_current_relative(2000);

            break;
        }
        case HIO_SAM_M8Q_STATE_READ:
        {
            uint16_t bytes_available;

            if (!hio_i2c_memory_read_16b(self->_i2c_channel, self->_i2c_address, 0xfd, &bytes_available))
            {
                self->_state = HIO_SAM_M8Q_STATE_ERROR;
            ...
            while (bytes_available != 0)
            {
                ...
                {
                    if (_hio_sam_m8q_feed(self, self->_ddc_buffer[i]))
                    {
                        self->_state = HIO_SAM_M8Q_STATE_UPDATE;
                    }
                }
            ...
            if (self->_state == HIO_SAM_M8Q_STATE_UPDATE)
            {
                goto start;
            }

            hio_scheduler_plan_current_relative(100);

            break;
        }
        case HIO_SAM_M8Q_STATE_UPDATE:
        {
            self->_state = HIO_SAM_M8Q_STATE_READ;
            ...
            if (self->_event_handler != NULL)
            {
                self->_event_handler(self, HIO_SAM_M8Q_EVENT_UPDATE, self->_event_param);
            }

            goto start;
        }
        case HIO_SAM_M8Q_STATE_STOP:
        {
            self->_running = false;

            if (!_hio_sam_m8q_disable(self))
            {
                self->_state = HIO_SAM_M8Q_STATE_ERROR;
            ...
            if (self->_event_handler != NULL)
            {
                self->_event_handler(self, HIO_SAM_M8Q_EVENT_STOP, self->_event_param);
            }
...
static bool _hio_sam_m8q_feed(hio_sam_m8q_t *self, char c)
{
    ...
    if (c == '\r' || c == '\n')
    {
        if (self->_line_length != 0)
        {
            ...
            {
                if (_hio_sam_m8q_parse(self, self->_line_buffer))
                {
                    ret = true;
                }
            }
            ...
            if (!self->_configured)
            {
                if (_hio_sam_m8q_send_config(self))
                {
