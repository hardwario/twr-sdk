...
#define HIO_LED_STRIP_NULL_TASK HIO_SCHEDULER_MAX_TASKS + 1

static uint32_t _hio_led_strip_wheel(int position);
static void _hio_led_strip_get_heat_map_color(float value, float *red, float *green, float *blue);

void hio_led_strip_init(hio_led_strip_t *self, const hio_led_strip_driver_t *driver, const hio_led_strip_buffer_t *buffer)
{
    memset(self, 0x00, sizeof(hio_led_strip_t));
    self->_buffer = buffer;
    self->_driver = driver;
    self->_effect.task_id = HIO_LED_STRIP_NULL_TASK;
...
void hio_led_strip_set_pixel(hio_led_strip_t *self, int position, uint32_t color)
{
    ...
    {
        hio_led_strip_set_pixel_rgbw(self, position, color >> 24, color >> 16, color >> 8, color);
    }
    else
    {
...
bool hio_led_strip_set_rgbw_framebuffer(hio_led_strip_t *self, uint8_t *framebuffer, size_t length)
{
    ...
    if (self->_buffer->type == HIO_LED_STRIP_TYPE_RGBW)
    {
...
void hio_led_strip_fill(hio_led_strip_t *self, uint32_t color)
{
    ...
    {
        hio_led_strip_set_pixel(self, i, color);
    }
}
...
{
    if (self->_effect.task_id != HIO_LED_STRIP_NULL_TASK)
    {
        hio_scheduler_unregister(self->_effect.task_id);

        self->_effect.task_id = HIO_LED_STRIP_NULL_TASK;
    }
}

static void _hio_led_strip_effect_done(hio_led_strip_t *self)
{
    hio_led_strip_effect_stop(self);

    if (self->_event_handler != NULL)
    {
        self->_event_handler(self, HIO_LED_STRIP_EVENT_EFFECT_DONE, self->_event_param);
    }
}

static void _hio_led_strip_effect_test_task(void *param)
{
    hio_led_strip_t *self = (hio_led_strip_t *)param;
    ...
    {
        _hio_led_strip_effect_done(self);
        return;
    }

    hio_scheduler_plan_current_relative(self->_effect.wait);
}
...
void hio_led_strip_effect_test(hio_led_strip_t *self)
{
    ...
    hio_led_strip_fill(self, 0x00000000);

    self->_effect.task_id = hio_scheduler_register(_hio_led_strip_effect_test_task, self, 0);
}

static void _hio_led_strip_effect_rainbow_task(void *param)
{
    hio_led_strip_t *self = (hio_led_strip_t *)param;
    ...
    for(int i = 0; i< self->_buffer->count; i++) {
        hio_led_strip_set_pixel(self, i, _hio_led_strip_wheel((i + self->_effect.round) & 255));
    }
...
void hio_led_strip_effect_rainbow(hio_led_strip_t *self, hio_tick_t wait)
{
    ...
    self->_effect.task_id = hio_scheduler_register(_hio_led_strip_effect_rainbow_task, self, 0);
}

static void _hio_led_strip_effect_rainbow_cycle_task(void *param)
{
    hio_led_strip_t *self = (hio_led_strip_t *)param;
    ...
    for(int i = 0; i< self->_buffer->count; i++) {
        hio_led_strip_set_pixel(self, i, _hio_led_strip_wheel(((i * 256 / self->_buffer->count) + self->_effect.round) & 255));
    }
...
void hio_led_strip_effect_rainbow_cycle(hio_led_strip_t *self, hio_tick_t wait)
{
    ...
    self->_effect.task_id = hio_scheduler_register(_hio_led_strip_effect_rainbow_cycle_task, self, 0);
}

static void _hio_led_strip_effect_color_wipe_task(void *param)
{
    hio_led_strip_t *self = (hio_led_strip_t *)param;
    ...
    hio_led_strip_set_pixel(self, self->_effect.led++, self->_effect.color);
...
void hio_led_strip_effect_color_wipe(hio_led_strip_t *self, uint32_t color, hio_tick_t wait)
{
    ...
    self->_effect.task_id = hio_scheduler_register(_hio_led_strip_effect_color_wipe_task, self, 0);
}

static void _hio_led_strip_effect_theater_chase_task(void *param)
{
    hio_led_strip_t *self = (hio_led_strip_t *)param;
    ...
    for (int i = self->_effect.led ; i < self->_buffer->count; i += 3) {
        hio_led_strip_set_pixel(self, i, self->_effect.color);    //turn every third pixel on
    }
...
void hio_led_strip_effect_theater_chase(hio_led_strip_t *self, uint32_t color, hio_tick_t wait)
{
    ...
    self->_effect.task_id = hio_scheduler_register(_hio_led_strip_effect_theater_chase_task, self, 0);
}

static void _hio_led_strip_effect_theater_chase_rainbow_task(void *param)
{
    hio_led_strip_t *self = (hio_led_strip_t *)param;
    ...
    for (int i = self->_effect.led; i < self->_buffer->count; i += 3) {
        hio_led_strip_set_pixel(self, i, _hio_led_strip_wheel((i + self->_effect.round) % 255) );    //turn every third pixel on
    }
...
void hio_led_strip_effect_theater_chase_rainbow(hio_led_strip_t *self, hio_tick_t wait)
{
    ...
    self->_effect.task_id = hio_scheduler_register(_hio_led_strip_effect_theater_chase_rainbow_task, self, 0);
}

static void hio_led_strip_effect_stroboscope_task(void *param)
{
    hio_led_strip_t *self = (hio_led_strip_t *)param;
    ...
    hio_led_strip_fill(self, color);

    hio_led_strip_write(self);
...
void hio_led_strip_effect_stroboscope(hio_led_strip_t *self, uint32_t color, hio_tick_t wait)
{
    ...
    self->_effect.task_id = hio_scheduler_register(hio_led_strip_effect_stroboscope_task, self, 0);
}

void _hio_led_strip_effect_icicle_task(void *param)
{
    hio_led_strip_t *self = (hio_led_strip_t *)param;
    ...
    for (int i = self->_effect.led; (i < self->_effect.led + length) && i < self->_buffer->count; i++) {
        ...
        hio_led_strip_set_pixel(self, i, 0x00);
    }
    ...
    for (int i = self->_effect.led; (i < self->_effect.led + length) && i < self->_buffer->count; i++) {
        ...
        hio_led_strip_set_pixel_rgbw(self, i, r, g, b, w); // 0x20000000
...
void hio_led_strip_effect_icicle(hio_led_strip_t *self, uint32_t color, hio_tick_t wait)
{
    ...
    self->_effect.task_id = hio_scheduler_register(_hio_led_strip_effect_icicle_task, self, 0);
}

static void _hio_led_strip_effect_pulse_color_task(void *param)
{
    hio_led_strip_t *self = (hio_led_strip_t *)param;
    ...
    for (int i = 0; i < self->_buffer->count; i++)
    {
        hio_led_strip_set_pixel_rgbw(self, i, r, g, b, w);
    }
...
void hio_led_strip_effect_pulse_color(hio_led_strip_t *self, uint32_t color, hio_tick_t wait)
{
    ...
    self->_effect.task_id = hio_scheduler_register(_hio_led_strip_effect_pulse_color_task, self, 0);
}
...
void hio_led_strip_thermometer(hio_led_strip_t *self, float temperature, float min, float max, uint8_t white_dots, float set_point, uint32_t color)
{
    ...
    {
        _hio_led_strip_get_heat_map_color((float)i / self->_buffer->count, &red, &green, &blue);
