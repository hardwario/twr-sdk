...
#define _HIO_HDC2080_DELAY_RUN 50
#define _HIO_HDC2080_DELAY_INITIALIZATION 50
#define _HIO_HDC2080_DELAY_MEASUREMENT 50

static void _hio_hdc2080_task_interval(void *param);

static void _hio_hdc2080_task_measure(void *param);
...
void hio_hdc2080_init(hio_hdc2080_t *self, hio_i2c_channel_t i2c_channel, uint8_t i2c_address)
{
    ...
    self->_task_id_interval = hio_scheduler_register(_hio_hdc2080_task_interval, self, HIO_TICK_INFINITY);
    self->_task_id_measure = hio_scheduler_register(_hio_hdc2080_task_measure, self, _HIO_HDC2080_DELAY_RUN);

    self->_tick_ready = _HIO_HDC2080_DELAY_RUN;
...
bool hio_hdc2080_get_humidity_percentage(hio_hdc2080_t *self, float *percentage)
{
    ...
    if (!hio_hdc2080_get_humidity_raw(self, &raw))
    {
        return false;
    }
...
bool hio_hdc2080_get_temperature_celsius(hio_hdc2080_t *self, float *celsius)
{
    ...
    if (!hio_hdc2080_get_temperature_raw(self, &raw))
    {
        return false;
    }
...
start:
    ...
    switch (self->_state)
    {
        case HIO_HDC2080_STATE_ERROR:
        {
            ...
            {
                self->_event_handler(self, HIO_HDC2080_EVENT_ERROR, self->_event_param);
            }

            self->_state = HIO_HDC2080_STATE_INITIALIZE;

            return;
        }
        case HIO_HDC2080_STATE_INITIALIZE:
        {
            self->_state = HIO_HDC2080_STATE_ERROR;

            if (!hio_i2c_memory_write_8b(self->_i2c_channel, self->_i2c_address, 0x0e, 0x80))
            {
                goto start;
            }

            self->_state = HIO_HDC2080_STATE_MEASURE;

            self->_tick_ready = hio_tick_get() + _HIO_HDC2080_DELAY_INITIALIZATION;
        ...
        case HIO_HDC2080_STATE_MEASURE:
        {
            self->_state = HIO_HDC2080_STATE_ERROR;

            if (!hio_i2c_memory_write_8b(self->_i2c_channel, self->_i2c_address, 0x0f, 0x07))
            {
                goto start;
            }

            self->_state = HIO_HDC2080_STATE_READ;

            hio_scheduler_plan_current_from_now(_HIO_HDC2080_DELAY_MEASUREMENT);

            return;
        }
        case HIO_HDC2080_STATE_READ:
        {
            self->_state = HIO_HDC2080_STATE_ERROR;

            uint8_t reg_interrupt;

            if (!hio_i2c_memory_read_8b(self->_i2c_channel, self->_i2c_address, 0x04, &reg_interrupt))
            {
                goto start;
            }
            ...
            if (!hio_i2c_memory_read_16b(self->_i2c_channel, self->_i2c_address, 0x02, &self->_reg_humidity))
            {
                goto start;
            }
            ...
            self->_state = HIO_HDC2080_STATE_UPDATE;

            goto start;
        }
        case HIO_HDC2080_STATE_UPDATE:
        {
            self->_measurement_active = false;

            if (self->_event_handler != NULL)
            {
                self->_event_handler(self, HIO_HDC2080_EVENT_UPDATE, self->_event_param);
            }
