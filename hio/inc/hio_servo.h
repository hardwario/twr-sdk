#ifndef _HIO_SERVO_H
#define _HIO_SERVO_H

#include <hio_pwm.h>

//! @addtogroup hio_servo hio_servo
//! @brief Driver for Servo
//! @{

#define HIO_SERVO_NO_ANGLE      255
#define HIO_SERVO_UNKNOWN_ANGLE 250
#define HIO_SERVO_DEFAULT_MIN   544
#define HIO_SERVO_DEFAULT_MAX   2400

//! @brief Servo instance

typedef struct
{
    hio_pwm_channel_t _channel;
    uint16_t _min;
    uint16_t _max;
    uint8_t _angle;

} hio_servo_t;

//! @endcond

//! @brief Initialize Servo
//! @param[in] self Instance
//! @param[in] channel PWM Channel

void hio_servo_init(hio_servo_t *self, hio_pwm_channel_t channel);

//! @brief Set angle
//! @param[in] self Instance
//! @param[in] angle Angle in degrees, 0 to 180

void hio_servo_set_angle(hio_servo_t *self, uint8_t angle);

//! @brief Get angle
//! @param[in] self Instance

uint8_t hio_servo_get_angle(hio_servo_t *self);

//! @brief Set pulse limits
//! @param[in] self Instance
//! @param[in] min_us Min pulse in us (Default 544us)
//! @param[in] max_us Max pulse in us (Default 2400us)

void hio_servo_set_pulse_limits(hio_servo_t *self, uint16_t min_us, uint16_t max_us);

//! @brief Set Microseconds
//! @param[in] self Instance
//! @param[in] us Microseconds

void hio_servo_set_microseconds(hio_servo_t *self, uint16_t us);

//! @}

#endif // _HIO_SERVO_H
