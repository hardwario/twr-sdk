...
int _read(int file, char *ptr, int len)
{
    ...
    case STDIN_FILENO:
        ...
        {
#if STDIN_USART == 1
            while ((USART1->ISR & SPI_FLAG_RXNE) == (uint16_t)RESET)
            {
            }
            char c = (char)(USART1->RDR & (uint16_t)0x01FF);
#elif STDIN_USART == 2
            while ((USART2->ISR & SPI_FLAG_RXNE) == (uint16_t)RESET)
            {
            }
            char c = (char)(USART2->RDR & (uint16_t)0x01FF);
#elif STDIN_USART == 3
            while ((USART3->ISR & SPI_FLAG_RXNE) == (uint16_t)RESET)
            {
            }
...
int _write(int file, char *ptr, int len) {
    ...
    case STDOUT_FILENO: /*stdout*/
        for (n = 0; n < len; n++) {
            ...
            USART1->RDR = (*ptr++ & (uint16_t)0x01FF);
#elif  STDOUT_USART == 2
            while ((USART2->ISR & USART_FLAG_TC) == (uint16_t) RESET) {
            }
            USART2->RDR = (*ptr++ & (uint16_t) 0x01FF);
#elif  STDOUT_USART == 3
            while ((USART3->ISR & USART_FLAG_TC) == (uint16_t)RESET) {}
            USART3->RDR = (*ptr++ & (uint16_t)0x01FF);
#endif
        }
        break;
